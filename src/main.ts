...
import {
	electronRemote,
	type ElectronWindow as RemoteElectronWindow,
} from "./utils/electron";
import {
	observeWindows,
	showWindows,
...
interface FileManagerWithCreate {
...
export default class TrayPlugin extends Plugin {
	...
	getCurrentWindow(): RemoteElectronWindow {
		return electronRemote.getCurrentWindow();
	}

	getWindows(): RemoteElectronWindow[] {
		return getWindows();
	}
	...
	setLaunchOnStartup(): void {
		...
		electronRemote.app.setLoginItemSettings({
	...
	addQuickNote(): void {
		...
		const date = (
			moment as unknown as () => { format: (p: string) => string }
		)().format(pattern);
		...
		const fileManager = this.app.fileManager as unknown as FileManagerWithCreate;
		fileManager
			.createNewMarkdownFile(root, name)
			.then((file) => leaf.openFile(file, openMode))
			.catch((error: Error) =>
				logger.error("Error creating quick note: " + error.message),
			);
	...
	closeVault(): void {
		...
		const obsidianWindows = electronRemote.BrowserWindow.getAllWindows();
